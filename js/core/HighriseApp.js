...
export class HighriseApp {
    ...
    async init() {
        try {
            ...
            // Agent System - load all 227 roster agents
            ...
            this._updateLoading(75, 'Onboarding 227 agents...');
    ...
    _setupEvents() {
        ...
        // Dashboard integration - FlightDeck button opens 2D dashboard at floor
        eventBus.on('floorPanel:flightdeckLink', (data) => {
            const floorId = data.floorId || data.projectId || '';
            window.open('/dashboard/#floor=' + floorId, '_blank');
        });

        // Dashboard integration - Agent history opens 2D dashboard at agent
        eventBus.on('agentPanel:viewHistory', (data) => {
            if (data.agentId) {
                window.open('/dashboard/#agent=' + data.agentId, '_blank');
            }
        });
